...
let rec mapi f list =
  let rec mapi' f list mapi_list index= (* mapi_list je ubistvu acc *)
    match list with
    | [] -> reverse mapi_list
    | x :: xs -> 
...
let rec mapi f list =
  let rec mapi' f list acc index= (* lepse napisana *)
    match list with
    | [] -> reverse acc
...
let rec zip list1 list2 =
  let rec zip' list1 list2 acc =
    ...
    | [], [] -> reverse acc
    | _, [] -> failwith "Exception: Failure Different lengths of input lists."
    | [], _ -> failwith "Exception: Failure Different lengths of input lists."
...
let rec unzip_tlrec list=
  let rec unzip_tlrec' list acc bcc =
    match list with
    | [] -> (reverse acc, reverse bcc)
    | (x, y) :: xs -> unzip_tlrec' xs (x :: acc) (y :: bcc) 
  in
  unzip_tlrec' list [] []
...
let rec loop cond f x =
  let rec loop' cond f x acc =
    if cond(x) then loop' (f x) acc else 
